...
{
    public class ActivitySnowController : Controller
    {
        //上海圣诞雪花的应用
        // GET: /ActivitySnow/
        ...
        public ActivitySnowController(ISnowLogRepository SnowLogRepository,
        ...
        /// <returns></returns>
        public ActionResult SnowIndex(Guid SnowID, string UserWexinID = "")
        {
        ...
        /// <returns></returns>
        public ActionResult SnowRank(Guid SnowID, string UserWexinID = "")
        {
            var Snow = SnowRepository.GetByKey(SnowID);
        ...
        /// <summary>
        /// 雪花游戏 有四种奖励 日票 夜票 杯子 一年份报纸
        /// 由于奖品起初设定的是一个月，奖品的数量正好是一天两个，所以将日票和夜票归于一类，杯子和报纸归于一类
        /// </summary>
        /// <param name="SnowID"></param>
        /// <param name="UserWexinID"></param>
        /// <returns></returns>
        public ActionResult SnowGame(Guid SnowID, string UserWexinID = "")
        {
            var snow = SnowRepository.GetByKey(SnowID);
            if (DateTime.Now < snow.StartDate || DateTime.Now > snow.EndDate.AddDays(1))
            {
            ...
            var SnowItemDatas = new List<SnowItemData>();
            var IsAward = 0;
            //获取当日中奖日志数
            ...
            var NewSnowItemDatas = new List<SnowItemData>();
            for (int i = 0; i < 10; i++)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult SaveSnowGameData(string datas, string UserName, string Phone, Guid SnowID)
        {
            SnowErrorLog sel = new SnowErrorLog();
            var snow = SnowRepository.GetByKey(SnowID);
            var SaveDatas = JsonConvert.DeserializeObject<List<SnowItemData>>(datas);
            //防止重复提交
            ...
            var SessionDatas = (List<SnowItemData>)Session["SnowItemDatas"];
