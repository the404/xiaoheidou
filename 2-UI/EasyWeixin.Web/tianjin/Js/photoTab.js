    ...
    $.fn.photoTab = function (options) {
        ...
        function next(speed) {
            $imgBox.animate({'margin-left': '-100%'}, speed, function () {
				...
				$imgBox.find('li').eq(0).html('<img src="'+s+'" class="realPic" /><span class="load"></span>');
				$imgBox.find('li').eq(0).css("background-image","url("+s+")");
				$imgBox.find('li').eq(0).find("img")[0].onload = function(){
					$(this).parent("li").find("span").css("opacity",0);
				}
				/*$imgBox.find('li').eq(0).css("background-image","url("+s+")");*/
        ...
        function prev(speed) {
			var ss=options.preFn();
			$imgBox.find('li:last').html('<img src="'+ss+'" class="realPic" /><span class="load"></span>');
			$imgBox.find('li:last').css("background-image","url("+ss+")");
			$imgBox.find('li:last').find("img")[0].onload = function(){
				$(this).parent("li").find("span").css("opacity",0);
			}
