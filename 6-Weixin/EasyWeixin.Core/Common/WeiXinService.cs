...
namespace EasyWeixin.Core.Common
{
    public class WeiXinService
    {
        ...
        /// <returns></returns>
        public static weather GetWeatherInfo(string cityCode)
        {
        ...
        public static string GetTrainString(string TrainCode)
        {
            Train t = GetTrain(TrainCode);
            string str = "";
            str += "您查询的列车车次为:" + TrainCode + ",本车次时刻如下:\n\n";
            str += t.trainScheduleBody[0].content[1] + "起点站 " + "开车时间:" + t.trainScheduleBody[0].content[4] + "\n\n";
            for (int i = 1; i < t.trainScheduleBody.Count - 1; i++)
            {
                var tsb = t.trainScheduleBody[i];
                str += tsb.content[1] + " 时间:" + tsb.content[2] + " 到站时间:" + tsb.content[3] + " 开车时间:" + tsb.content[4] + "\n\n";
            }
            str += t.trainScheduleBody[t.trainScheduleBody.Count - 1].content[1] + "终点站 " + "到站时间:" + t.trainScheduleBody[t.trainScheduleBody.Count - 1].content[2] + " " + t.trainScheduleBody[t.trainScheduleBody.Count - 1].content[3] + " \n\n";
            return str;
        }

        public static Train GetTrain(string TrainCode)
        {
            TrainCode = TrainCode.ToUpper();
            string dt = ((DateTime.Now.ToUniversalTime().Ticks - 621355968000000000) / 10000000).ToString();
            string url = "http://train.qunar.com/qunar/checiInfo.jsp?ex_track=&q=" + TrainCode + "&format=json&cityname=123456&ver=" + dt + "&callback=XQScript_5";
            string jsonstring = NetworkHelper.SendRequest(url).Replace("XQScript_5(", "");
            jsonstring = jsonstring.Substring(0, jsonstring.Length - 2);
            jsonstring = replaceStr(jsonstring, "ticket", TrainCode);
            jsonstring = replaceStr(jsonstring, "tinfo", TrainCode);
            Train t = JsonConvert.DeserializeObject<Train>(jsonstring);
            List<trainSchedule> trainScheduleBody = t.trainScheduleBody.OrderBy(o => Int16.Parse(o.content[6].Replace("公里", ""))).ToList();
            t.trainScheduleBody = trainScheduleBody;
            return t;
        }

        public static string replaceStr(string str, string newval, string oldval)
        {
            int index = str.IndexOf(oldval);
            string pre = str.Substring(0, index + oldval.Length).Replace(oldval, newval);
            return pre + str.Substring(index + oldval.Length);
        }

        #endregion 火车查询(去哪儿网的接口)

        #region 航班查询

        public static string GetFlightString(string flightCode)
        {
            Flight f = GetFlight(flightCode);
            if (f.resultcode == "200")
            {
                string str = "您所查询的航班为" + flightCode + ",将由" + f.result[0].start + "开往" + f.result[0].end + ",此次航班起飞时间为" + f.result[0].DepTime + ",降落时间为" + f.result[0].ArrTime + ",";
                str += "总计时间为:" + f.result[0].FlyTime + "分钟";
                return str;
            }
            else if (f.resultcode == "202")
            {
                return "查询不到结果";
            }
            else
        ...
        public static Flight GetFlight(string FlightCode)
        {
            ...
            string url = "http://apis.juhe.cn/plan/s?name=" + FlightCode + "&date=" + dt + "&key=b4d10f33a75b13945bf57ef56f0c5ed2";
            string jsonstring = NetworkHelper.SendRequest(url);
            return JsonConvert.DeserializeObject<Flight>(jsonstring);
        }
        ...
        public static string GetJHTrainString(string JHTrainCode)
        {
            try
            {
                JHTrain j = GetJHTrain(JHTrainCode);
                if (j.resultcode == 200)
                {
                    string str = "您查询的列车车次为:" + JHTrainCode + ",本车次时刻如下:\n\n";
                    str += j.result.station_list[0].station_name + " 起点站 " + "开车时间:" + j.result.station_list[0].leave_time + "\n\n";
                    for (int i = 1; i < j.result.station_list.Count - 1; i++)
                    {
                        str += j.result.station_list[i].station_name + " 到达时间:" + j.result.station_list[i].arrived_time + " 开车时间:" + j.result.station_list[i].leave_time + "\n\n";
                    }
                    str += j.result.station_list[j.result.station_list.Count - 1].station_name + " 终点站 " + "到达时间:" + j.result.station_list[j.result.station_list.Count - 1].arrived_time + "\n\n";
                    return str;
                }
                else if (j.resultcode == 202)
                {
                    return "查询不到结果";
                }
                else
                {
                    return j.resultcode.ToString();
                }
            }
            catch (Exception)
            {
        ...
        public static JHTrain GetJHTrain(string JHTrainCode)
        {
            JHTrainCode = JHTrainCode.ToUpper();
            string url = "http://apis.juhe.cn/train/s?name=" + JHTrainCode + "&key=93bfd0e2eba63abc7d1d285b78a3ced9";
            string jsonstring = NetworkHelper.SendRequest(url);
            return JsonConvert.DeserializeObject<JHTrain>(jsonstring);
        }

        #endregion 火车查询（聚合网）
    }

    #region 火车查询类(去哪儿网的接口)
    ...
    public class tinfo
    {
        public string arriCity_py { set; get; }

        public string istmp { set; get; }

        public string code { set; get; }

        public string deptCity_py { set; get; }

        public string arriStation { set; get; }

        public string arriCity { set; get; }

        public string trainType { set; get; }

        public string deptTime { set; get; }

        public string deptStation { set; get; }

        public string deptCity { set; get; }

        public string arriTime { set; get; }
    ...
    {
        public tinfo tinfo { set; get; }
    }

    public class trainSchedule
    {
        public string mxl { set; get; }
    ...
    public class Train
    {
        public ticketInfo ticketInfo { set; get; }

        public List<trainSchedule> trainScheduleBody { set; get; }

        public trainInfo trainInfo { set; get; }

        public string[] trainScheduleHead { set; get; }
    }

    #endregion 火车查询类(去哪儿网的接口)

    #region 天气查询类

    public class weather
    {
        public virtual weatherinfo weatherinfo { set; get; }
    }
    ...
    public class weatherinfo
    {
        ...
        public string city_en { set; get; }

        public string date_y { set; get; }

        public string date { set; get; }

        public string week { set; get; }

        public string fchh { set; get; }
        ...
        public string tempF1 { set; get; }

        public string weather1 { set; get; }

        public string weather2 { set; get; }

        public string weather3 { set; get; }

        public string weather4 { set; get; }

        public string img1 { set; get; }

        public string img_title1 { set; get; }

        public string wind1 { set; get; }

        public string wind2 { set; get; }

        public string wind3 { set; get; }

        public string wind4 { set; get; }

        public string fl1 { set; get; }

        public string index { set; get; }

        public string index_d { set; get; }

        public string index48 { set; get; }

        public string index_uv { set; get; }

        public string index_xc { set; get; }

        public string index_tr { set; get; }

        public string index_co { set; get; }

        public string st1 { set; get; }

        public string index_cl { set; get; }

        public string index_ls { set; get; }

        public string index_ag { set; get; }
    }
    ...
    public class Flight
    {
        ...
        public List<FlightResult> result { set; get; }
    }
    ...
    public class FlightResult
    {
        ...
        /// 航空公司
        /// </summary>
        public string complany { set; get; }

        /// <summary>
        /// 机型
        /// </summary>
        public string AirModel { set; get; }

        /// <summary>
        /// 机年
        /// </summary>
        public string AirAge { set; get; }
        ...
        /// 预计起飞时间
        /// </summary>
        public string Dexpected { set; get; }

        /// <summary>
        /// 预计到达时间
        /// </summary>
        public string Aexpected { set; get; }

        /// <summary>
        /// 实时起飞时间
        /// </summary>
        public string Dactual { set; get; }

        /// <summary>
        /// 实时到达时间
        /// </summary>
        public string Aactual { set; get; }
        ...
        /// 准点率
        /// </summary>
        public string OnTimeRate { set; get; }
    ...
    public class JHTrain
    {
        ...
        public JHresult result { set; get; }
    }

    public class JHresult
    {
        public train_info train_info { set; get; }

        public List<station> station_list { set; get; }
    }
    ...
    public class station
    {
        ...
        /// 到达时间
        /// </summary>
        public string arrived_time { set; get; }
        ...
        /// 二等座
        /// </summary>
        public string ssoftSeat { set; get; }

        /// <summary>
        /// 一等座
        /// </summary>
        public string fsoftSeat { set; get; }

        /// <summary>
        /// 硬座
        /// </summary>
        public string hardSead { set; get; }

        /// <summary>
        /// 软座
        /// </summary>
        public string softSeat { set; get; }

        /// <summary>
        /// 硬卧
        /// </summary>
        public string hardSleep { set; get; }

        /// <summary>
        /// 软卧
        /// </summary>
        public string softSleep { set; get; }
    }
