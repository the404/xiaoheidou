...
namespace EasyWeixin.CommonAPIs
{
    ...
    {
        private static Dictionary<string, JsApiTicketBag> JsApiTicketCollection =
        ...
        public static JsApiTicketResult GetTicketResult(string appId, bool getNewTicket = false)
        {
            ...
            lock (accessTicketBag.Lock)
            {
                ...
                {
                    //已过期，重新获取
                    accessTicketBag.JsApiTicketResult = CommonApi.GetJsApi_Ticket(accessTicketBag.AppId, accessTicketBag.AppSecret);
                    accessTicketBag.ExpireTime = DateTime.Now.AddSeconds(accessTicketBag.JsApiTicketResult.expires_in - 1000);
                }
            }
